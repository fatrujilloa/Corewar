...
/*   Updated: 2020/02/03 00:50:22 by ftrujill         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    ft_and(t_cw *cw, t_process *prcs, int i, t_op op) //ARG, ARG, REG
{
    ...
    ft_printf("P   %u | %s %d %d r%d\n", prcs->nb + 1, op.name, arg.real_value[0], arg.real_value[1], arg.int_value[2]);
    prcs->pc = (prcs->pc + arg.total_size) % MEM_SIZE;
    prcs->carry = (ft_str_to_int(prcs->reg[arg.int_value[2] - 1], 
        REG_SIZE) == 0) ?  1: 0;
}
